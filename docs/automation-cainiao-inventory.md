...
这条命令会自动完成：登录 → 导出当天库存 → 导入系统 → 清理旧缓存 → 发送表格格式钉钉消息。
...
```mermaid
flowchart TD
    Start["Agent 执行: npm run sync:inventory:full"] --> CheckAuth{"检查 tests/.auth/cainiao.json"}
...
1. **查询当天日期**
   - `sync-cainiao-inventory.js` 中日期设置为**当天**（`new Date()`），不再使用"昨天"或"前天"。
...
1. **直接执行**：`npm run sync:inventory:full`
2. **如果报错 Cookie 失效**：先执行 `npm run test:login`，等待用户验证完成，再执行 `npm run sync:inventory:full`
3. **如果报错"文件日期不是今天"**：说明 `sync:inventory` 被错误调用，应改用 `npm run sync:inventory:full`
